...
public class ThongKeRepository {
    ...
    public List<ThongKeSanPhamHotResponse> thongKeSanPhamHot(int topN, LocalDate startDate, LocalDate endDate){
        String queryString = "SELECT sp.id, sp.ten AS tenSanPham, SUM(gd.so_tien_giao_dich) AS doanhThu " +
                ...
                "AND gd.ngay_thanh_toan BETWEEN :startDate AND :endDate " +
                "GROUP BY sp.id, sp.ten " +
                "ORDER BY doanhThu DESC " +
                "LIMIT :topN";

        List<Object[]> results = entityManager.createNativeQuery(queryString, "ThongKeSanPhamHotResponse")
                .setParameter("startDate", startDate)
                .setParameter("endDate", endDate)
                .setParameter("topN", topN)
                .getResultList();

//        List<ThongKeSanPhamHotResponse> dtos = new ArrayList<>();
//        for(Object[] result : results){
//            ThongKeSanPhamHotResponse dto = new ThongKeSanPhamHotResponse();
//            dto.setId(result[0] != null ? ((Number) result[0]).longValue() : 0L);
//            dto.setTenSanPham(result[1] != null ? ((String) result[1]) : "");
//            dto.setDoanhThu(result[2] != null ? ((BigDecimal) result[2]) : BigDecimal.ZERO);
//            dtos.add(dto);
//        }
//        return dtos;

        return results.stream().map(result -> new ThongKeSanPhamHotResponse(
                (Long) result[0],
                (String) result[1],
                (BigDecimal) result[2]
